...
- Privacy Center: controls exist for local JSON/ZIP export bundle, document
  deletion, AI correction/deletion, local connector disconnect and executed
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| Account/Auth | Invite-only login, session and password reset | Email, auth id, password handled by Supabase, user metadata, session/access token metadata | Admin invite, login/reset actions | Supabase Auth; frontend browser session storage managed by supabase-js; backend mirrors `sub` into SQLite `User.id` | Supabase and configured auth-email provider | Provider retention unknown | Full active-account/Auth deletion is implemented and locally tested; backup/provider retention remains open | Contract necessity and email-provider review | High | Target dashboard verification, backup policy, DPA/AVV | Yes |
| Billing/subscriptions | Sell and manage paid Avareno plans | Avareno user id, provider customer id, provider subscription id, provider price id, plan key, billing interval, status, period dates, cancel-at-period-end flag, safe webhook event id/type/status, invoice number/status/amount/currency/period dates, Stripe-hosted invoice/PDF URLs; provider may process email, invoice, tax/VAT and payment data | User chooses paid plan, provider checkout, provider webhook, customer portal | SQLite `PlanSubscription`, `BillingEvent`, `BillingInvoice`; future production DB; payment/card data and invoice PDFs stay with provider | Stripe subscription billing | Undefined until provider contract/dashboard review | Export/deletion must include local subscription state and invoice metadata; provider-side cancellation/deletion/invoice export process TODO | Contract necessity likely for paid subscription; VAT/tax/DPA/AVV and privacy policy disclosure need legal review | High | Stripe legal/tax/provider review, Checkout Sessions, webhooks, customer portal, invoice template, cancellation flow, retention policy, production RLS | Yes |
...
| Documents/uploads | Store manuals, warranties, support files, private docs | Uploaded files, filenames, MIME, paths, extracted text/json | Explicit document upload | Authenticated local storage/SQLite plus private Supabase Storage policies | Supabase Storage; AI/OCR disabled in invite beta | Undefined | Signed/authenticated download, strict upload policy, document deletion, local ZIP export and full account deletion exist | Contract necessity and special-category screening for Vault-like docs | High | Target-project verification, malware scanning, retention and production export jobs | Yes |
...
| AI extraction / Memory Build | Future structured extraction; disabled for invite beta | Existing extracted fields may remain in legacy local data; no beta provider payload | No active beta trigger | Existing SQLite fields only | None in invite beta | Not applicable while disabled | Existing fields can be corrected/deleted and account deletion removes them | New review required before enablement | High | Provider choice, DPA/AVV, no-training settings, disclosure and prompt retention | Yes |
...
| Avareno Connect / Home Graph | Import/link selected external data and centralize smart-home devices | Provider registry metadata, selected/mock records, device IDs, room names, capabilities, status, commands, tokens/secrets, sync logs, future Device Passport links to receipts/warranties/manuals | Home Graph provider selection, safe mock connect preview/confirm, Connect provider/sandbox/smart-home actions, Smart Home Connect Hub UI, local Avareno Bridge metadata preview | Frontend registry/mock Device Passport types; SQLite smart-home tables for confirmed mock imports; env token for SmartThings/local Bridge test; frontend receives metadata only, never provider secrets | SmartThings/Samsung, Bambu/LAN, Home Assistant via local Avareno Bridge MVP, Matter planned, future providers | Undefined | Disconnect/token deletion plan only | Consent, scopes, provider DPA/AVV, token processing | High | Encrypted token store, OAuth/pairing, revocation, SSRF tests, safe sync logs, rate limits, Device Passport export/deletion mapping | Yes |
...
| Deletion/export | User rights and account lifecycle | All user-owned categories plus auth/storage/provider data | Authenticated Privacy Center export/delete actions | JSON/ZIP export plus synchronous server deletion orchestration | Supabase Auth/Database/Storage; future providers when enabled | Backup/provider retention undefined | Local/Supabase database, Storage, files and Auth deletion tested; production export jobs/backups remain open | Data subject rights timing/process review | High | Production export jobs/provider receipts, backup policy and future provider revocation | Yes |
...
- Supabase Storage/DB: versioned local configuration and migrations with
  controlled two-user verification; intended beta-project state still needs
...
- Production export jobs and backup deletion/restoration remain open; active
  account deletion is implemented and locally integrated with Supabase.
- Intended beta-project RLS/Storage state still requires deployment QA.
...
- Production data export jobs and backup deletion/restoration policy.
- Retention and backup policy.
- Intended beta-project dashboard/RLS/Storage verification.
