...
- Backend: FastAPI API with local SQLite MVP database and local `/uploads` file storage.
...
- Privacy Center: foundation/plan endpoints exist; export and deletion return `501` and are not complete.
...
| Receipts/invoices | Store proof of purchase and structured purchase facts | File, filename, MIME, merchant, price, date, warranty, extracted text/json | Upload document, extract receipt | Local `/uploads`, SQLite `Document`; future storage buckets | Future AI/OCR provider if enabled | Undefined | Document deletion plan only, no execution | Legal basis and sensitive document handling | High | Private storage, signed URLs, file deletion, size/MIME scanning, extraction correction | Yes |
| Documents/uploads | Store manuals, warranties, support files, private docs | Uploaded files, filenames, MIME, paths, extracted text/json | Document upload | Local static `/uploads`, SQLite `Document`; future Supabase Storage | Storage provider, future AI/OCR | Undefined | Not complete | Consent/contract and special-category screening for Vault-like docs | High | Access-controlled storage, encryption, malware scanning, retention, delete/export | Yes |
...
| Deletion/export | User rights and account lifecycle | All user-owned categories plus auth/storage/provider data | Privacy Center export/delete requests | Foundation endpoints only; return `501` | Supabase, storage, connectors, AI provider | Undefined | Not implemented | Data subject rights timing/process review | High | Export archive, delete orchestrator, auth deletion, file deletion, provider revocation | Yes |
...
- Local files: project `/uploads`, served statically by FastAPI.
...
- Public/static local `/uploads` without per-file auth.
...
- Data export and account/document deletion execution.
